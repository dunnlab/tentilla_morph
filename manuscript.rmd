...
grid.newpage()
grid.table(morphdata)

#Make a heatmap
heatdata = as.matrix(castmean_logs[,-1])
rownames(heatdata) = castmean_logs$Species
heatmap(heatdata, scale = "column", cexCol = 0.5, col=terrain.colors(20))
```
...
#Soft vs Hard
soft_hard = cbind((diet$`Copepod diet`+diet$`Amphipod diet`+diet$`Crustacean diet` + diet$`Decapod diet` + diet$`Ostracod diet`), (diet$`Fish diet`+diet$`Polychaete diet`+diet$`Chaetognath diet`+diet$`Mollusc diet`))
...
#LOGISTIC REGRESSION
copepod = diet$`Copepod diet`
names(copepod) = rownames(diet)
heteroneme_elongation = dprunedmatrix_logs$heteroneme_elongation
names(heteroneme_elongation) = dprunedmatrix_logs$Species
copepod = copepod[match(dprunedTree$tip.label, names(copepod))]
heteroneme_elongation = heteroneme_elongation[match(dprunedTree$tip.label, names(heteroneme_elongation))]
dat = data.frame(heteroneme_elongation, copepod)
dat = dat[which(!is.nan(dat$heteroneme_elongation)),]
null = dat
null$heteroneme_elongation = rep(1, nrow(dat))
xtree = drop.tip(dprunedTree, which(!(dprunedTree$tip.label %in% rownames(dat))))
logit = phyloglm(copepod~heteroneme_elongation,phy=xtree,data=dat,boot=100)
logit = phyloglm(copepod~,phy=xtree,data=null,boot=100)
summary(logit)
coef(logit)
vcov(logit)
nlogit = glm(copepod~heteroneme_elongation,data=dat, family="binomial")
...
for(prey in 1:ncol(logit_diet)){
  for(char in 2:ncol(dprunedmatrix_logs)){
    ...
    datIJ = data.frame(preytype = CH_J, character = CH_I)
      ...
      print(logit)
      phylogit = phyloglm(preytype~character,phy=treeIJ,data=datIJ,boot=10, btol=100)
      print(phylogit)
...
write.csv(logits, "logits.csv", row.names = F)
