...
![\label{morphodiet}Phylomorphospace of continuous morphological characters with inapplicable states transformed to zeroes, overlapped with polygons conservatively defining the space occupied by each feeding guild. ](~/tentilla_morph/Organismal_paper/figures/morphospace_diet.png){height=600px}

In order to examine the occupation structure of the morphospace across all siphonophore species in the dataset, we cast a PCA on the data transforming inapplicable states (due to absence of character) to zeroes. This allows us to accommodate species with many missing characters (such as cystonects), and to account for common absences as morphological similarities. In this ordination, PC1 explains 49.99% of variation (aligned with characters of the involucrum, terminal filament, and elastic strands) and PC2 explains 25.65% of variation (aligned with heteroneme number and haploneme length). When superimposing feeding guilds onto the morphospace (Fig. \@ref(morphodiet)), we find that the morphospaces of each feeding guild are barely overlapping in PC1 and PC2. A phylogenetic MANOVA showed that feeding guilds explain 22.09% of the variance in those dimensions, an outcome significantly distinct from the expectation under neutral evolution (p-value = 0.0196). In addition, a morphological disparity analysis accounting for phylogenetic structure shows that the morphospace of fish specialists is significantly broader than that of generalists and other specialists. This is due to the large morphological disparities between cystonects and piscivorous euphysonects. The superimposition of feeding guilds on the morphospace predicts the diet of *Erenna laciniata* to be piscivorous, *Lensia conoidea* and *Gymnopraia lapislazula* to be small crustacean eaters, *Lilyopsis fluoracantha* and *Physophora hydrostatica* to be generalists, and *Marrus claudanielis*, *Physophora gilmeri*, and *Halistemma rubrum* to be large crustacean specialists.
...
hypdiet_full = c("Mixed","Mixed","Gelatinous","Gelatinous","Mixed","Large crustacean","Small crustacean","Small crustacean","Small crustacean","Small crustacean","Fish","Fish","Mixed","Small crustacean","Large crustacean","Small crustacean","Large crustacean","Fish","Large crustacean","Large crustacean","Large crustacean","Fish","Fish","Small crustacean","Small crustacean","Fish","Small crustacean","Small crustacean")
dpruned_full = fullraw_nozeroes[which(fullraw_nozeroes$Species%in%rownames(diet)),]
