...
#Cnidoband size evolution
cnisize = sharedmean_logs$cnidomic_index
names(cnisize) = sharedmean_logs$Species
treei = drop.tip(ultram, which(!(ultram$tip.label %in% names(cnisize))))
Phenogram(cnisize, as.data.frame(cnisize), treei)

#SURFACE AREA : VOLUME
SAV_haploneme = surface_ellipsoid(sharedmeans$Haploneme.free.length..um., sharedmeans$Haploneme.width..um.)/volume_ellipsoid(sharedmeans$Haploneme.free.length..um., sharedmeans$Haploneme.width..um.)
names(SAV_haploneme) = sharedmeans$Species
SAV_heteroneme = surface_ellipsoid(sharedmeans$Heteroneme.free.length..um., sharedmeans$Heteroneme.width..um.)/volume_ellipsoid(sharedmeans$Heteroneme.free.length..um., sharedmeans$Heteroneme.width..um.)
names(SAV_heteroneme) = sharedmeans$Species
SAV_haploneme = SAV_haploneme[which(!is.nan(SAV_haploneme))]
SAV_heteroneme = SAV_heteroneme[which(!is.nan(SAV_heteroneme))]
SAVhet_tree = drop.tip(ultram, which(!(ultram$tip.label %in% names(SAV_heteroneme))))
Phenogram(SAV_heteroneme, as.data.frame(SAV_heteroneme), SAVhet_tree)
Phenogram(SAV_haploneme, as.data.frame(SAV_haploneme), hap_tree)
...
#C = sharedlogs[,c(-1,-3)] %>% .[which(!is.na(rowSums(.[,-1]))),] ##Doesn't work
...
#PIC correlations by character, to get significance
PICp = as.data.frame(matrix(nrow=ncol(C), ncol=ncol(C)))
colnames(PICp) = colnames(C)
rownames(PICp) = colnames(C)
for(i in 2:ncol(sharedmean_logs)){
  Ci = sharedmean_logs[,i]
  names(Ci) = sharedmean_logs$Species
  Ci = Ci[!is.na(Ci)]
  for(j in 2:ncol(sharedmean_logs)){
    Cj = sharedmean_logs[,j]
    names(Cj) = sharedmean_logs$Species
    Cj = Cj[!is.na(Cj)]
    Cj = Cj[which(names(Cj) %in% names(Ci))]
    Ci = Ci[which(names(Ci) %in% names(Cj))]
    if(i!=j){
      ctree = drop.tip(ultram, which(!(ultram$tip.label %in% names(Cj))))
      class(ctree)<-"phylo"
      pic_i = pic(Ci, ctree)
      pic_j = pic(Cj, ctree)
      cor.test(pic_i, pic_j)$p.value %>% round3() -> PICp[i-1,j-1]
    }
    else PICp[i-1,j-1]<-NA
  }
}
#write.csv(PICp, "PICcorrelations_pvalues.csv")
...
CH_I = dprunedCat$Rhopalonemes #same result with desmonemes
...
phylogit = phyloglm(preytype~character,phy=treeIJ,data=datIJ,boot=10, btol=10)
    print(phylogit)
c("Soft specilization", "Haploneme SA:V", phylogit$n, phylogit$aic, coef(summary(phylogit))[2,6], coef(summary(phylogit))[2,1], logit$aic, coef(summary(logit))[2,4], coef(summary(logit))[2,1])
```
...
#Plot mirror trees QUANT
X = dprunedmatrix_logs$Haploneme.row.number
names(X) = dprunedmatrix_logs$Species
X=X[!(is.na(X))]
Y = diet$`Decapod diet`
names(Y) = rownames(diet)

X = dprunedmatrix_logs$coiledness
names(X) = dprunedmatrix_logs$Species
X<-X[!(is.nan(X))]
Y = diet$`Decapod diet`
#Y=softORhard[,1]
...
X = Sprunedmatrix_logs$Haploneme.row.number..um.
names(X) = Sprunedmatrix_logs$Species
Y = selectivity$Selectivity.Decapod.larvae #PP$`Relative decapod consumption`
names(Y) = rownames(selectivity)
layout(matrix(1:3,1,3),widths=c(0.44,0.12,0.44))
par(cex=0.5)
#pdf("deliverables/Hvol_copepod.pdf",width=10,height=6)
OX = contMap(Sprunedtree,X,sig=1,legend=1, plot = F, outline = F)
OX$cols[1:length(OX$cols)] = colorRampPalette(c("blue","red"), space="Lab")(length(OX$cols))
plot(OX,ftype="off")
ylim<-c(1-0.12*(length(Sprunedtree$tip.label)-1),length(Sprunedtree$tip.label))
plot.new(); plot.window(xlim=c(-0.1,0.1),ylim=ylim)
text(rep(0,length(Sprunedtree$tip.label)), 1:length(Sprunedtree$tip.label),Sprunedtree$tip.label)
OY = contMap(Sprunedtree,Y, sig=1,legend=1, plot = F,  outline = F)
OY$cols[1:length(OY$cols)] = colorRampPalette(c("orange","green"), space="Lab")(length(OY$cols))
plot(OY, direction="leftwards", ftype = "off")
...
#ldamtrix = sharedmean_logs[which(rowNAs<20),]
ldamtrix = ldamtrix[which(!is.nan(rowSums(ldamtrix[,-1]))),]
#ldamtrix = ldamtrix[,which(!is.nan(colSums(ldamtrix[,-1])))]
#rowNAs = apply(ldamtrix[,-1], 1, function(x) sum(is.na(x)))
#colNAs = apply(ldamtrix[,-1], 2, function(x) sum(is.nan(x)))
ldamtrix = ldamtrix[which(rowNAs<1),]
#ldamtrix = ldamtrix[,which(colNAs<1)]
...
#Copepods
ldamtrix = cbind(baselda, prunediets[match(baselda$Species, rownames(prunediets)), "Copepod diet"])
...
predictionset=predictionset[which(!(predictionset$Species %in% c("Thermopalia taraxaca", "Nectadamas richardi", "Halistemma cupulifera", "Cardianecta parchelion"))),]
...
#Fish
ldamtrix = cbind(baselda, prunediets[match(baselda$Species, rownames(prunediets)), "Fish diet"])
...
#Decapods
ldamtrix = cbind(baselda, prunediets[match(baselda$Species, rownames(prunediets)), "Decapod diet"])
...
ldamtrix = cbind(baselda, prunediets[match(baselda$Species, rownames(prunediets)), 1])
...
#Predicitions on uncharacterized taxa transforming NAs to zeroes
...
#Ecology prediciton?
kinediet = diet[which(rownames(diet) %in% kine$Species),]
kinesel = selectivity[which(rownames(selectivity) %in% kine$Species),]
kinehypdiet = hypdiet[which(names(hypdiet) %in% kine$Species)]
...
```{r rphylopars_and_varpart}
library(Rphylopars)
combinedata = data.frame(sharedmean_logs,diet[match(sharedmean_logs$Species, rownames(diet)),])
names(combinedata)[1]<-"species"
is.nan.data.frame <- function(x)
do.call(cbind, lapply(x, is.nan))
combinedata[is.nan.data.frame(combinedata)] <- NA
#PhyloParsALL = phylopars(combinedata[,c(1,2,4:22,24,27:42)], ultram, pheno_error = F, pheno_correlated = T)
#PhyloParsALL = phylopars(combinedata[,c(1,32:42)], ultram, pheno_error = F, pheno_correlated = T)

#How to make combined imputations

###PCOA variance partitionaing method
phylomatrix = pcoa(cophenetic(dprunedTree))$vectors
phylomatrix = phylomatrix[match(rownames(diet), rownames(phylomatrix)),]
nastozeroes = dprunedmatrix_logs
nastozeroes[is.nan(dprunedmatrix_logs)]<-0
nastozeroes = nastozeroes[,match(rownames(diet), nastozeroes$Species)]
dietdist <- vegdist(diet, "bray") %>% as.matrix()
varpart(nastozeroes[,-1], phylomatrix[,1:3], diet)
glmatrix_1 = cbind(nastozeroes, phylomatrix)
glmatrix_2 = cbind(nastozeroes, diet)
glmatrix_3 = cbind(nastozeroes, diet, phylomatrix)
glm(Cnidoband.length..um.~Axis.1+Axis.2+Axis.3+Axis.4+Axis.6+Axis.8+Axis.9+Axis.11+Axis.14, data=glmatrix_1[-1,]) -> GLM_1
1-(GLM_1$deviance/GLM_1$null.deviance)
predict.glm(GLM_1, newdata = as.data.frame(glmatrix_1[1,]), se.fit = T)
glm(Cnidoband.length..um.~`Copepod diet`+`Fish diet`+`Amphipod diet`+`Decapod diet`+`Polychaete diet`+`Mollusc diet`+`Crustacean diet`+`Chaetognath diet`+`Ostracod diet`+`Gelatinous diet`, data=glmatrix_2[-1,]) -> GLM_2
1-(GLM_2$deviance/GLM_2$null.deviance)
predict.glm(GLM_2, newdata = as.data.frame(glmatrix_2[1,]), se.fit = T)
glm(Cnidoband.length..um.~Axis.1+Axis.2+Axis.3+Axis.4+Axis.6+Axis.8+Axis.9+Axis.11+Axis.14+`Copepod diet`+`Fish diet`+`Amphipod diet`+`Decapod diet`+`Polychaete diet`+`Mollusc diet`+`Crustacean diet`+`Chaetognath diet`+`Ostracod diet`+`Gelatinous diet`, data=glmatrix_3[-1,]) -> GLM_3
1-(GLM_2$deviance/GLM_2$null.deviance)
predict.glm(GLM_3, newdata = as.data.frame(glmatrix_3[1,]), se.fit = T)

varpart(nastozeroes$coiledness, phylomatrix[,c(1:4,6,8,9,11,14)], diet)
rda(nastozeroes$coiledness, phylomatrix[,c(1:4,6,8,9,11,14)]) %>% summary()

###Phylopars methods
pparsmatrix = cbind(nastozeroes$Species, nastozeroes$Cnidoband.length..um., diet)
names(pparsmatrix)[1]<-"species"
pparsmatrix$`nastozeroes$Cnidoband.length..um.` [1]<-NA
PhyloParsALL = phylopars(pparsmatrix, dprunedTree, pheno_error = F, pheno_correlated = T)
PhyloParsALL$anc_recon["Agalma elegans",]
pparscontrol = pparsmatrix[,1:2]
PhyloPars_control = phylopars(pparscontrol, dprunedTree, pheno_error = F, pheno_correlated = T)
PhyloPars_control$anc_recon["Agalma elegans",]

###PGLS predictions do not take into account the phylogenetic position of the new taxon###
pglsmatrix = glmatrix_2
names(pglsmatrix) = str_replace_all(names(pglsmatrix)," ", "_")
rownames(pglsmatrix) = pglsmatrix$Species
pglm_1 = gls(Cnidoband.length..um. ~ Copepod_diet+Fish_diet+Amphipod_diet+Decapod_diet+Polychaete_diet+Mollusc_diet+Crustacean_diet+Chaetognath_diet+Ostracod_diet+Gelatinous_diet, data=pglsmatrix[,-1], correlation = corBrownian(phy=dprunedTree))
pglm_control = gls(Cnidoband.length..um. ~ Copepod_diet+Fish_diet+Amphipod_diet+Decapod_diet+Polychaete_diet+Mollusc_diet+Crustacean_diet+Chaetognath_diet+Ostracod_diet+Gelatinous_diet, data=pglsmatrix[,-1], correlation = corBrownian(phy=rtree(length(dprunedTree$tip.label))))
predict(pglm_1, newdata = pglsmatrix[1,])
predict(pglm_control, newdata = pglsmatrix[1,])
```
